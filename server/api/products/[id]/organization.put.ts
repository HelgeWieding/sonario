...
export default defineEventHandler(async (event) => {
  ...
  if (organizationId && organizationId !== auth.orgId) {
    badRequest('Cannot assign product to an organization you are not actively using')
  }

  let product
  if (organizationId) {
    product = await productRepository.assignToOrganization(productId, user.id, organizationId)
  } else {
    product = await productRepository.removeFromOrganization(productId, user.id)
  }
