...
export function useProducts() {
  ...
  const productsOrgContext = useState<string | null>(
    "products-org-context",
    () => null
  );
  ...
  async function fetchProducts() {
    ...
    try {
      ...
      if (selectedProduct.value) {
        const currentStillExists = products.value.some(
  ...
  // Fetch products with redirect if empty (for page setup)
  async function fetchProductsWithRedirect(): Promise<boolean> {
    await fetchProducts();
  ...
  async function refreshProducts() {
    ...
    try {
      ...
      if (selectedProduct.value) {
        const updated = products.value.find(
          (p) => p.id === selectedProduct.value?.id
        );
        if (updated) {
