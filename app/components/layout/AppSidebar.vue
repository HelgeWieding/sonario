...
const { buildOrgRoute, buildProductRoute } = useOrgSlug();
...
const currentSlug = computed(
    () => route.params.productSlug as string | undefined,
);
...
function getProductNavigation(productSlug: string) {
    return [
        ...
        {
            name: "Messages",
            href: buildProductRoute(productSlug, "messages"),
            icon: "envelope",
        },
        {
            name: "Contacts",
            href: buildProductRoute(productSlug, "contacts"),
            icon: "users",
        },
        {
            name: "Feedback",
            href: buildProductRoute(productSlug, "feedback"),
            icon: "chat",
        },
        {
            name: "Settings",
            ...
            children: [
                ...
                {
                    name: "Gmail",
                    href: buildProductRoute(productSlug, "settings/gmail"),
                },
                {
                    name: "Help Scout",
                    href: buildProductRoute(productSlug, "settings/helpscout"),
                },
            ],
        },
    ];
    ...
    if (item.children) {
        return item.children.some(
            (child) =>
                ...
                route.path.startsWith(child.href + "/"),
        );
    }
        ...
        <nav class="flex-1 p-3 space-y-1 overflow-hidden scrollbar-thin">
            ...
            <div class="pt-5 mt-5">
                ...
                >
                    ...
                    <NuxtLink
                        :to="
                            buildProductRoute(product.slug, 'feature-requests')
                        "
                        :class="[
                    ...
                    >
                        <template
                            v-for="navItem in getProductNavigation(
                                product.slug,
                            )"
                            :key="navItem.name"
                        >
