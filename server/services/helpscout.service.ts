...
export class HelpScoutService {
  ...
  async getMailboxes(): Promise<HelpScoutMailbox[]> {
    try {
      const response = await this.fetch<{
        _embedded: { mailboxes: HelpScoutMailbox[] };
      }>("/mailboxes");
  ...
  ): Promise<HelpScoutConversation | null> {
    try {
      return await this.fetch<HelpScoutConversation>(
  ...
  async getConversationThreads(
    ...
    try {
      const response = await this.fetch<{
        _embedded: { threads: HelpScoutThread[] };
  ...
  async createDraftReply(options: {
    ...
    const makeRequest = async (token: string): Promise<Response> => {
      return fetch(
        `${this.baseUrl}/conversations/${options.conversationId}/reply`,
        {
          method: "POST",
          headers: {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.text();
        console.error(
          `Failed to create draft reply for conversation ${options.conversationId}:`,
          error
        );
        return false;
      }
      ...
      console.log(
        `Draft reply created for conversation ${options.conversationId}`
      );
      return true;
    } catch (error) {
